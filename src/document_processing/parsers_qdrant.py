    ...
    try:
        with open(file_path, "rb") as docx_file:
            ...
            for table in soup.find_all('table'):
                ...
                # ИСПРАВЛЕНИЕ: Теперь оборачиваем таблицу в тег div, чтобы парсер её заметил
                if parsed_table:
            ...
            def save_chunk():
                ...
                # Overlap: оставляем последний абзац для следующего чанка
                last_paragraph = current_chunk_content[-1] if current_chunk_content else ""
                if 20 < len(last_paragraph) < 500: current_chunk_content = [last_paragraph]
...
# ==========================================
# ТРЕК 2: HTML (Остался без изменений)
# ==========================================
